#!/bin/bash
# Deploy Gallery Service as AWS Lambda Function

set -e

# Configuration
AWS_REGION="ap-southeast-2"
FUNCTION_NAME="video-forge-gallery-service"
RUNTIME="nodejs20.x"
HANDLER="lambda-handler.handler"
MEMORY_SIZE=512
TIMEOUT=30
IAM_ROLE="CAB432-Lambda-Role"  # Must have Lambda, RDS, S3, Secrets Manager permissions
...
echo -e "${GREEN}================================================${NC}"
echo -e "${GREEN}Gallery Service Lambda Deployment${NC}"
echo -e "${GREEN}================================================${NC}"
echo ""
...
# Create .env for Lambda (will use environment variables instead)
...
# Create zip file
cd ${DEPLOY_DIR}
zip -r ../gallery-service-lambda.zip . -q
cd ..
rm -rf ${DEPLOY_DIR}

echo -e "${GREEN}✓ Deployment package created: gallery-service-lambda.zip${NC}"
echo -e "  Size: $(du -h gallery-service-lambda.zip | cut -f1)"
echo ""
...
if [ -z "$ROLE_ARN" ]; then
    echo -e "${YELLOW}IAM role not found. Please create it first:${NC}"
    echo ""
    echo "aws iam create-role --role-name ${IAM_ROLE} \\"
    ...
    echo "aws iam attach-role-policy --role-name ${IAM_ROLE} --policy-arn arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
    echo "aws iam attach-role-policy --role-name ${IAM_ROLE} --policy-arn arn:aws:iam::aws:policy/AmazonS3FullAccess"
    echo "aws iam attach-role-policy --role-name ${IAM_ROLE} --policy-arn arn:aws:iam::aws:policy/SecretsManagerReadWrite"
    echo "aws iam attach-role-policy --role-name ${IAM_ROLE} --policy-arn arn:aws:iam::aws:policy/AmazonSSMReadOnlyAccess"
    echo "aws iam attach-role-policy --role-name ${IAM_ROLE} --policy-arn arn:aws:iam::aws:policy/AmazonVPCFullAccess"
    echo ""
    exit 1
fi
...
# Get VPC configuration for RDS access
...
echo "1. Create API Gateway HTTP API to trigger this Lambda"
echo "2. Configure environment variables (DB connection, S3 bucket, etc.)"
...
echo ""
echo -e "${YELLOW}Create API Gateway:${NC}"
echo "See: setup-lambda-api-gateway.sh"
echo ""

# Cleanup
rm -f gallery-service-lambda.zip
