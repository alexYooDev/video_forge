...
class AWSConfigSetup {
  ...
  async setupConfiguration() {
    ...
    const parameterValues = {
      ...
      '/video-forge/config/s3-bucket-name': process.env.S3_BUCKET_NAME || 'video-forge-storage',
      '/video-forge/auth/cognito-user-pool-id': process.env.COGNITO_USER_POOL_ID,
      '/video-forge/auth/cognito-client-id': process.env.COGNITO_CLIENT_ID,
      '/video-forge/cache/redis-host': process.env.REDIS_HOST || 'localhost',
      '/video-forge/cache/redis-port': process.env.REDIS_PORT || '6379',
      '/video-forge/cache/enabled': process.env.CACHE_ENABLED || 'true'
    };
    ...
    const secretValues = {
      ...
      '/video-forge/auth/cognito-client-secret': process.env.COGNITO_CLIENT_SECRET
    };
