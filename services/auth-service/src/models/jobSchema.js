...
const jobSchema = Joi.object({
  inputSource: Joi.alternatives()
    .try(
      ...
      Joi.string().pattern(/^gallery\//) // Gallery service S3 keys (gallery/userId/*)
    )
    .required()
    .messages({
      'alternatives.match': 'Input source must be a valid URL, S3 key (videos/* or gallery/*), or local sample file',
